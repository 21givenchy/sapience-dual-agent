...
const sdk_1 = require("@dome-api/sdk");
const groq_sdk_1 = __importDefault(require("groq-sdk"));
const dotenv = __importStar(require("dotenv"));
dotenv.config();
class DomeGroqTester {
    constructor() {
        this.dome = new sdk_1.DomeClient({
     ...
     */
    async fetchPolymarketMarkets(limit = 5) {
        try {
            ...
            const markets = marketsArray.slice(0, limit).map((market) => ({
     ...
     */
    async fetchPolymarketMarketBySlug(slug) {
        try {
     ...
     */
    async fetchKalshiMarkets(limit = 5) {
        try {
...
- Apply market microstructure analysis: consider spread, volume, and price efficiency`;
    ...
    displayForecast(forecast) {
        ...
        if (forecast.market.slug)
            console.log(`🔗 Slug: ${forecast.market.slug}`);
    ...
    async run() {
        ...
        try {
            ...
            console.log('═'.repeat(80));
            const specificMarket = await this.fetchPolymarketMarketBySlug('will-gavin-newsom-win-the-2028-us-presidential-election');
...
exports.default = DomeGroqTester;
//# sourceMappingURL=test-dome-groq.js.map